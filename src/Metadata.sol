...
/**
* @title : On-chain svg & json generator
 */
contract Metadata {
	address public Gov;
	...
	function image(bytes calldata _name) public view returns(string memory) {
		(string[] memory _arr, string memory _list) = toHexColor(abi.encodePacked(keccak256(_name)));
		string memory _stop;
		for (uint i = 0; i < 10; i++) {
		...
		return string.concat("data:image/svg+xml;base64,", encode64(bytes(string.concat(a, _stop, b, _list, c, _list, d, string(_name), ".ubi.eth</text></svg>"))));
	}

	function generate(uint Id) public view returns(string memory) {
		/*//bytes calldata _name, address _owner, uint _expiry, uint _ethFees
		string memory _name2 = string.concat(string(_name), ".ubi.eth");
		return string.concat(
			'data:text/plain,{"name":"', _name2, '",',
			'"description":"UBI Supporters NFT Club",',
			'"external_url": "https://', string(_name), '.ubi.istest.eth.limo",',
			'"image":"', image(_name), '",',
			'"expiry":"', toString(_expiry), '",',
			'"total_fees_paid":"', toString(_ethFees), '",',
			'"owner":"', toHexString(abi.encodePacked(_owner)), '",',
			attrib(_name),
			'}');
		*/
	}
	...
	function attrib(bytes calldata _name) internal pure returns(string memory) {
		//string memory _pat;
		//uint id = keccak256(_name);
		/*for(uint i = 10; i > 0; i--){
		    if(id % i == 0){
